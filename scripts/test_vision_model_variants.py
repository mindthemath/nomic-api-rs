...
def test_direct_accuracy(fp32_session, fp16_session, quantized_session, tokenizer_name="vision"):
    ...
    for name, url in test_images:
        try:
            ...
            print(f"{name:<30} {fp16_diff:<12.6f} {fp16_cos_sim:<12.6f} {quantized_diff:<12.6f} {quantized_cos_sim:<12.6f}")
...
def test_batch_size_sensitivity(fp32_session, fp16_session, quantized_session):
    ...
    print(f"{'':<15} {'Max Diff':<12} {'Avg Cos Sim':<12} {'Max Diff':<12} {'Avg Cos Sim':<12}")
    print("-" * 80)
    ...
    for batch_size in [1, 2, 4, 8]:
        ...
        # Compare
        fp16_diffs = [np.abs(fp32_embs[i] - fp16_embs[i]).max() for i in range(batch_size)]
        fp16_cos_sims = [cosine_similarity(fp32_embs[i], fp16_embs[i]) for i in range(batch_size)]
        
        quantized_diffs = [np.abs(fp32_embs[i] - quantized_embs[i]).max() for i in range(batch_size)]
        quantized_cos_sims = [cosine_similarity(fp32_embs[i], quantized_embs[i]) for i in range(batch_size)]
        ...
        results.append((batch_size, fp16_max_diff, fp16_avg_cos_sim, quantized_max_diff, quantized_avg_cos_sim))
        print(f"{batch_size:<15} {fp16_max_diff:<12.6f} {fp16_avg_cos_sim:<12.6f} {quantized_max_diff:<12.6f} {quantized_avg_cos_sim:<12.6f}")
...
def test_batching_interference(fp32_session, fp16_session, quantized_session):
    ...
    for name, session in [("FP32", fp32_session), ("FP16", fp16_session), ("Quantized", quantized_session)]:
        # Single inference
        ...
        print(f"{name:<15} {diff_1:<20.6f} {diff_2:<20.6f} {cos_sim_1:<15.6f} {cos_sim_2:<15.6f}")
...
def main():
    ...
    if uint8_path.exists() and uint8_path.stat().st_size != quantized_path.stat().st_size:
