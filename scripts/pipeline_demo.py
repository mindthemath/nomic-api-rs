...
def main():
    ...
    print(f"  Compressed size: {len(image_bytes):,} bytes ({len(image_bytes)/1024:.1f} KB)")
    ...
    print(f"    Black [0,0,0] → [{-MEAN[0]/STD[0]:.3f}, {-MEAN[1]/STD[1]:.3f}, {-MEAN[2]/STD[2]:.3f}]")
    print(f"    White [1,1,1] → [{(1-MEAN[0])/STD[0]:.3f}, {(1-MEAN[1])/STD[1]:.3f}, {(1-MEAN[2])/STD[2]:.3f}]")
    ...
    for dim in [768, 512, 256, 128, 64]:
        ...
        print(f"  dim={dim:3d}: first 3 values = [{truncated_norm[0]:.4f}, {truncated_norm[1]:.4f}, {truncated_norm[2]:.4f}]")
