...
def test_text_batch_quantized(url: str):
    ...
    resp = requests.post(
        f"{url}/txt/batch",
        json={"inputs": ["Hello world"], "dim": 768, "prefix": "search_query"},
    )
    if resp.status_code == 200:
    ...
    if resp.status_code == 400:
        ...
        if (
            "batching is not supported" in error_msg.lower()
            or "cross-sample interference" in error_msg.lower()
            or "exceeds maximum allowed batch size" in error_msg.lower()
        ):
...
def test_image_batch(url: str):
    ...
    resp = requests.post(
        f"{url}/img/batch",
        json={"contents": [f"data:image/png;base64,{img_base64}"] * 2, "dim": 768},
    )
    if resp.status_code == 200:
        data = resp.json()
        if len(data.get("embeddings", [])) == 2:
