...
def embed_batch(session: ort.InferenceSession, encodings: list) -> list[np.ndarray]:
    ...
    # L2 normalize each
    return [emb / np.linalg.norm(emb) if np.linalg.norm(emb) > 0 else emb for emb in embs]
...
def test_batch_size_sensitivity(fp32_session, fp16_session, tokenizer):
    ...
    for batch_size in [1, 2, 4, 8]:
        ...
        cos_sims = [cosine_similarity(fp32_embs[i], fp16_embs[i]) for i in range(batch_size)]
...
def test_length_composition_sensitivity(fp32_session, fp16_session, tokenizer):
    ...
    for name, composition in test_cases:
        ...
        # Compare each embedding
        diffs = [np.abs(fp32_embs[i] - fp16_embs[i]).max() for i in range(len(composition))]
        cos_sims = [cosine_similarity(fp32_embs[i], fp16_embs[i]) for i in range(len(composition))]
...
def test_ordering_sensitivity(fp32_session, fp16_session, tokenizer):
    ...
    ref_cos_sims = [cosine_similarity(fp32_singles[i], fp16_singles[i]) for i in range(3)]
