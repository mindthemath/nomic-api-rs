...
def test_precision(precision: str, model, tokenizer, device: str):
    ...
    print(f"{'Different texts: text[0] single vs batch[0]':<40} {diff_1:<15.6f} {cos_sim_1:.6f} {status_1}")
    print(f"{'Different texts: text[1] single vs batch[1]':<40} {diff_2:<15.6f} {cos_sim_2:.6f} {status_2}")
...
def main():
    ...
    if len(results) > 1:
        ...
        if fp32 and fp16:
            print(f"{'Different texts diff (text 1)':<40} {fp32['different_text_diff_1']:<15.6f} {fp16['different_text_diff_1']:<15.6f} {'N/A':<15}")
            print(f"{'Different texts diff (text 2)':<40} {fp32['different_text_diff_2']:<15.6f} {fp16['different_text_diff_2']:<15.6f} {'N/A':<15}")
            print(f"{'Different texts cos sim (text 1)':<40} {fp32['different_text_cos_1']:<15.6f} {fp16['different_text_cos_1']:<15.6f} {'N/A':<15}")
            print(f"{'Different texts cos sim (text 2)':<40} {fp32['different_text_cos_2']:<15.6f} {fp16['different_text_cos_2']:<15.6f} {'N/A':<15}")
            print(f"{'Large batch max diff':<40} {fp32['large_batch_diff']:<15.6f} {fp16['large_batch_diff']:<15.6f} {'N/A':<15}")
            print(f"{'Large batch cos sim':<40} {fp32['large_batch_cos']:<15.6f} {fp16['large_batch_cos']:<15.6f} {'N/A':<15}")
        
        if fp32 and bf16:
            print(f"\n{'Metric':<40} {'FP32':<15} {'BF16':<15}")
            print("-" * 70)
            print(f"{'Different texts diff (text 1)':<40} {fp32['different_text_diff_1']:<15.6f} {bf16['different_text_diff_1']:<15.6f}")
            print(f"{'Different texts diff (text 2)':<40} {fp32['different_text_diff_2']:<15.6f} {bf16['different_text_diff_2']:<15.6f}")
            print(f"{'Different texts cos sim (text 1)':<40} {fp32['different_text_cos_1']:<15.6f} {bf16['different_text_cos_1']:<15.6f}")
            print(f"{'Different texts cos sim (text 2)':<40} {fp32['different_text_cos_2']:<15.6f} {bf16['different_text_cos_2']:<15.6f}")
            print(f"{'Large batch max diff':<40} {fp32['large_batch_diff']:<15.6f} {bf16['large_batch_diff']:<15.6f}")
            print(f"{'Large batch cos sim':<40} {fp32['large_batch_cos']:<15.6f} {bf16['large_batch_cos']:<15.6f}")
    ...
    if len(results) > 0:
        ...
        print(f"\nPyTorch results:")
        if fp32_result:
            print(f"  FP32: {fp32_result['different_text_diff_1']:.6f} max diff, {fp32_result['different_text_cos_1']:.6f} cos sim")
        if fp16_result:
            print(f"  FP16: {fp16_result['different_text_diff_1']:.6f} max diff, {fp16_result['different_text_cos_1']:.6f} cos sim")
        if bf16_result:
            print(f"  BF16: {bf16_result['different_text_diff_1']:.6f} max diff, {bf16_result['different_text_cos_1']:.6f} cos sim")
    ...
    if fp16_result and fp16_result['different_text_diff_1'] < 0.0001:
        ...
        print("   Half-precision does NOT cause the same issues as ONNX INT8 quantization.")
