...
async def benchmark_batch_size(
    ...
    if endpoint == "txt":
        payload = {
            "inputs": [f"Test text {i} for batch size {batch_size}" for i in range(batch_size)],
            "dim": 768,
            "prefix": "search_query",
        }
    else:  # img
    ...
    if endpoint == "txt" and total_tokens_list:
        ...
        throughput_tokens_per_sec = (avg_tokens_per_batch * 1000.0 / avg_latency) if avg_latency > 0 else 0
...
async def run_benchmark_suite(
    ...
    if early_stop:
        print(f"Early stopping: enabled (stops if {degradation_threshold*100:.0f}% worse than best)")
    else:
    ...
    async with aiohttp.ClientSession() as session:
        for batch_size in batch_sizes:
            ...
            result = await benchmark_batch_size(session, base_url, endpoint, batch_size, num_runs, warmup)
            if "error" in result:
            ...
            print(f"   Total batch latency: {total_latency:.2f}ms | Per-item: {per_item_latency:.2f}ms/item | Throughput: {throughput_str}")
            ...
            if early_stop and len(results) > 1:
                if best_per_item_latency is None:
                    ...
                    if per_item_latency < best_per_item_latency:
                        improvement = ((best_per_item_latency - per_item_latency) / best_per_item_latency) * 100
                        best_per_item_latency = per_item_latency
                        best_throughput = throughput_val
                        best_batch_size = batch_size
                        print(f"   🎯 New best! {improvement:.1f}% better per-item latency")
                    # Check if significantly worse
                    elif per_item_latency > best_per_item_latency * (1 + degradation_threshold):
                        degradation = ((per_item_latency / best_per_item_latency - 1) * 100)
                        print(f"   ⚠️  Performance degraded: {degradation:.1f}% worse than best")
                        print(f"   🛑 Stopping early (best was batch_size={best_batch_size} with {best_per_item_latency:.2f}ms/item)")
                        break
...
def print_results_table(results: List[Dict], endpoint: str):
    ...
    if len(results) > 1:
        ...
        if endpoint == "txt" and results[0].get('throughput_tokens_per_sec'):
            best_throughput = max(results, key=lambda x: x.get("throughput_tokens_per_sec", 0) or 0)
            throughput_val = best_throughput.get('throughput_tokens_per_sec', 0)
            throughput_unit = "tok/s"
            throughput_alt = f" ({best_throughput['throughput_items_per_sec']:.2f} items/s)"
        else:
        ...
        if endpoint == "txt" and best_latency.get('throughput_tokens_per_sec'):
            print(f"  {best_latency['throughput_tokens_per_sec']:.2f} tok/s ({best_latency['throughput_items_per_sec']:.2f} items/s)")
        else:
...
def save_markdown(results: List[Dict], endpoint: str, output_file: str):
    ...
    with open(output_file, "w") as f:
        ...
        f.write("## Results\n\n")
        f.write("| Batch | Avg Latency (ms) | Std Dev | P50 | P95 | P99 | Throughput (items/s) | RPS | Success | Speedup |\n")
        f.write("|-------|-----------------|---------|-----|-----|-----|---------------------|-----|---------|--------|\n")
        ...
        if len(results) > 1:
            ...
            f.write("\n## Summary\n\n")
            f.write(f"- **Best throughput**: batch_size={best_throughput['batch_size']} "
                   f"({throughput_val:.2f} {throughput_unit})\n")
            f.write(f"- **Best per-item latency**: batch_size={best_latency['batch_size']} "
...
def main():
    ...
    batch_sizes = sorted(list(set([b for b in batch_sizes if b <= args.max_batch_size * 2])))
    ...
    try:
        ...
        if info_resp.status_code == 200:
            ...
            if txt_model:
                ...
                if 'quantized' in txt_model.lower() and args.endpoint in ['txt', 'both']:
                    print(f"  ⚠️  WARNING: Quantized text model detected - batching disabled!")
                    print(f"     Use FP32 model for batching: TXT_MODEL=models/txt/model.onnx make run-benchmark")
            if img_model:
    ...
    if args.endpoint in ["txt", "both"]:
        txt_results = asyncio.run(
            run_benchmark_suite(
                args.url, "txt", batch_sizes, args.runs, args.warmup,
                early_stop=early_stop, degradation_threshold=args.degradation_threshold
            )
        )
        if txt_results:
            ...
            if args.markdown:
                save_markdown(txt_results, "txt", args.markdown.replace(".md", "_txt.md"))
    
    if args.endpoint in ["img", "both"]:
        img_results = asyncio.run(
            run_benchmark_suite(
                args.url, "img", batch_sizes, args.runs, args.warmup,
                early_stop=early_stop, degradation_threshold=args.degradation_threshold
            )
        )
        if img_results:
            ...
            if args.markdown:
                save_markdown(img_results, "img", args.markdown.replace(".md", "_img.md"))
    ...
    if all_results:
        ...
        for endpoint, results in all_results.items():
            if results:
                if endpoint == "txt" and results[0].get('throughput_tokens_per_sec'):
                    best = max(results, key=lambda x: x.get("throughput_tokens_per_sec", 0) or 0)
                    throughput_str = f"{best.get('throughput_tokens_per_sec', 0):.1f} tok/s"
                else:
                    best = max(results, key=lambda x: x["throughput_items_per_sec"])
                    throughput_str = f"{best['throughput_items_per_sec']:.1f} items/s"
                print(f"  /{endpoint}/batch: Optimal batch_size={best['batch_size']} ({throughput_str})")
