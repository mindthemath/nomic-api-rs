...
def main():
    ...
    if "FP16" in results and "INT8" in results:
        ...
        print(
            f"  Speed: INT8 is {speed_ratio:.2f}x {'faster' if speed_ratio < 1.0 else 'slower'} than FP16"
        )
        print(
            f"  Size: INT8 is {1/size_ratio:.2f}x smaller than FP16 ({int8_size:.1f}MB vs {fp16_size:.1f}MB)"
        )
        ...
        if speed_ratio < 0.85 and size_ratio < 0.6:
            print(
                f"  Use INT8: Faster ({speed_ratio:.2f}x) and smaller ({1/size_ratio:.2f}x)"
            )
            print(
                f"  Trade-off: 94% accuracy vs 99.9999% (acceptable for most use cases)"
            )
        elif speed_ratio > 1.1:
