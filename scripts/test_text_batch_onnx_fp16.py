...
def test_model(model_path: Path, model_name: str):
    ...
    cos_sim_1 = np.dot(single_emb1, batch_embs[0]) / (np.linalg.norm(single_emb1) * np.linalg.norm(batch_embs[0]))
    cos_sim_2 = np.dot(single_emb2, batch_embs[1]) / (np.linalg.norm(single_emb2) * np.linalg.norm(batch_embs[1]))
...
def main():
    ...
    if len(results) >= 2:
        ...
        if "fp16" in results:
            fp16 = results["fp16"]
            if fp16['max_diff_same_len'] < 0.0001 and fp16['max_diff_different'] < 0.0001:
                ...
                print("   Much better than INT8 quantized (~0.5), acceptable for most use cases.")
            elif fp16['max_diff_same_len'] < results.get("quantized", {}).get("max_diff_same_len", 1.0) * 0.5:
