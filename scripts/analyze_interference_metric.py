...
def analyze_difference(emb1: np.ndarray, emb2: np.ndarray, label: str):
    ...
    print(
        f"  Dimensions differing >0.001: {num_different}/768 ({num_different/768*100:.1f}%)"
    )
...
def main():
    ...
    print("=" * 70)
    analyze_difference(
        single_emb1, single_emb2, "Different images (expected to be different)"
    )
    ...
    print(
        f"  - Cosine similarity of ~{cos_sim_1:.6f} means vectors are {cos_sim_1*100:.4f}% similar"
    )
