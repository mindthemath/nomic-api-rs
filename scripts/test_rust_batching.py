...
def test_batch_endpoint(base_url: str = "http://localhost:8080"):
    ...
    single_1 = requests.post(
        f"{base_url}/img/embed", json={"content": test_images[0]}, timeout=30
    ).json()

    single_2 = requests.post(
        f"{base_url}/img/embed", json={"content": test_images[1]}, timeout=30
    ).json()

    # Batch inference
    batch_response = requests.post(
        f"{base_url}/img/batch", json={"contents": test_images[:2]}, timeout=30
    ).json()
    ...
    for img in test_images:
        resp = requests.post(
            f"{base_url}/img/embed", json={"content": img}, timeout=30
        ).json()
        single_embs.append(resp["embedding"])

    # Batch inference
    batch_response = requests.post(
        f"{base_url}/img/batch", json={"contents": test_images}, timeout=30
    ).json()
    ...
    start = time.time()
    batch_response = requests.post(
        f"{base_url}/img/batch", json={"contents": large_batch}, timeout=60
    ).json()
    ...
    if all_normalized:
        ...
        print(
            f"❌ Some embeddings not normalized: {[f'{n:.4f}' for n in norms[:3]]}..."
        )
        return False
